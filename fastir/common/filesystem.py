...
class TSKFileSystem(FileSystem):
    ...
    def read_chunks(self, path_object):
        if path_object.name == '$UsnJrnl':
            founded_attr = None
            for attr in path_object.obj:
                if not hasattr(attr.info, 'name'):
                    continue
                if attr.info.name is None:
                    continue
                if attr.info.name == b'$J':
                    founded_attr = attr
                    break

            size = 0 if founded_attr is None else founded_attr.info.size
            attr_type = None if founded_attr is None else founded_attr.info.type
            attr_id = None if founded_attr is None else founded_attr.info.id
        else:
            size = path_object.obj.info.meta.size
            attr_type = attr_id = None

        offset = 0
        while offset < size:
            chunk_size = min(CHUNK_SIZE, size - offset)
            if attr_type is None and attr_id is not None:
                chunk = path_object.obj.read_random(offset, chunk_size, attr_type, attr_id)
            else:
